...
class RecommendationEngine:
    ...
    def _effort(model: ModelProfile, workload: WorkloadProfile) -> str:
        ...
        difficulty = workload.reasoning * 0.30 + workload.ambiguity * 0.22 + workload.agentic * 0.20 + workload.breadth * 0.18 + workload.coding * 0.10
    ...
    @staticmethod
    def _score(model: ModelProfile, workload: WorkloadProfile) -> float:
        c = model.capabilities
        need_total = workload.reasoning + workload.coding + workload.agentic + workload.breadth
        quality = (c.get("reasoning", 3) * workload.reasoning * 1.15 + c.get("coding", 3) * workload.coding * 1.10 + c.get("agentic", 3) * workload.agentic * 1.05 + c.get("long_horizon", 3) * workload.breadth * 0.80 + c.get("ambiguity", 3) * workload.ambiguity * 0.75)
        efficiency = c.get("speed", 3) * workload.latency_sensitivity * 0.55 + c.get("cost_efficiency", 3) * workload.cost_sensitivity * 0.70
        overkill = max(0.0, (sum(c.get(k, 3) for k in ("reasoning", "coding", "agentic")) / 3) - (need_total / 4))
    ...
    @staticmethod
    def _explain(model: ModelProfile, workload: WorkloadProfile, effort: str, mode: str) -> tuple[tuple[str, ...], tuple[str, ...]]:
        reasons: list[str] = []
        tradeoffs: list[str] = []
        ...
        if model.cost_efficiency >= 4:
            reasons.append("Good quality/cost fit for repeated use")
    ...
    def recommend(self, workload: WorkloadProfile, providers: Iterable[str] | None = None, include_limited: bool = False, top_n: int = 3) -> list[Recommendation]:
        ...
        for model in self.registry.candidates(providers=providers, include_limited=include_limited):
            ...
            score = self._score(model, workload)
            reasons, tradeoffs = self._explain(model, workload, effort, mode)
            scored.append(Recommendation(provider=model.provider, model_id=model.model_id, label=model.label, effort=effort, execution_mode=mode, score=round(score, 2), confidence=0.0, reasons=reasons, tradeoffs=tradeoffs))
