...
python -m tools.ai_model_advisor.cli feedback-add --feedback ~/.hive/model-feedback.jsonl --provider anthropic --model claude-sonnet-5 --effort high --execution single --outcome success --retries 0 --latency-seconds 42 --cost-usd 0.31 --input-tokens 18000 --output-tokens 2100 --cached-tokens 9000 --cache-creation-tokens 1200 --task-category implementation --task-id api-endpoint-17
...
Feedback is JSONL and stays local unless the user deliberately commits/uploads it. A record can include outcome (`success`, `partial`, `failure`), retries, latency, USD cost, input/output tokens, cache-read/cache-write token counts, Hive credits, task category, a stable task ID, an importer source ID, and a note.
...
Token/cache/credit telemetry is intentionally **diagnostic-only** today. It is retained to explain actual model behavior and to support future normalized experiments, but it does not change routing score. Lower token count is not automatically better: a stronger model may spend more tokens to solve a task correctly, and cache effectiveness is provider/workload dependent. `cached_tokens` and `cache_creation_tokens` are subsets/attributes of input usage and must never be added to `input_tokens` again when calculating totals.
...
- session `events.jsonl` contains `llm_turn_complete` events with model, input/output token usage, cache-read/cache-write usage, optional Hive credits, and provider-reported USD cost;
...
- a node that changed models mid-run is skipped instead of assigning its combined outcome/telemetry to one model;
...
- provider cost, token usage, cache usage, and Hive credits are aggregated across that node's LLM turns;
- missing/zero provider cost remains unknown rather than being interpreted as free;
- input/output totals do not double-count cache usage; cache-read and cache-creation counts are retained separately for diagnostics;
...
- median input/output tokens when present;
- median cache-read share of input and median cache-creation tokens when present;
- Hive-credit sample coverage and median when present;
...
The audit reads the same exported threshold constants used by the router itself. Threshold reporting therefore cannot silently drift away from scoring policy without breaking the regression tests. Token/cache/credit fields are shown as telemetry only and are not inputs to `quality_adjustment` or `efficiency_adjustment`.
