...
For OpenAI, the outgoing body must prove the requested effort via either `reasoning_effort` or `reasoning.effort`.

For Anthropic, the outgoing body must prove the requested effort via `output_config.effort`.

The outgoing model must also match the requested model. If the pinned LiteLLM version silently drops, rewrites, or cannot support the requested setting, the adapter exits non-zero and the experiment runner must not append model evidence.
...
Do not add a model/version allowlist just because the Advisor registry lists a model. Let the installed Hive/LiteLLM stack attempt the request, then require the captured post-transform body to prove that the requested model and effort were actually applied.
